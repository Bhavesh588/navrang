// Mock attachUser BEFORE requiring it
jest.mock('../middlewares/attachUser', () => {
  return (req, res, next) => {
    const authHeader = req.headers.authorization;
    
    // Only attach user for "fake_token" used in non-auth tests
    // For "invalid_token" and other tokens, don't attach user to test auth failures
    if (authHeader && authHeader.includes('fake_employee_token')) {
      req.user = {
        id: 2,
        role_id: 2,
        role_name: 'employee',
    ...
    } else if (authHeader && authHeader.includes('fake_other_employee_token')) {
      req.user = {
        id: 3,
        role_id: 2,
        role_name: 'employee',
    ...
    } else if (authHeader && authHeader.includes('fake_token')) {
      req.user = {
        id: 1,
        role_id: 1,
        role_name: 'admin',
    ...
    // For any other token (including 'invalid_token'), don't attach user
    // This allows auth tests to properly test 401 responses
